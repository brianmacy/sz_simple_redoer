      ...
      while True:
        ...
        if futures:
          ...
          for fut in done:
            ...
            if messages%INTERVAL == 0: # display rate stats
              ...
              print(f'Processed {messages} redo, {speed} records per second')
              prevTime=nowTime
            ...
            for fut, msg in futures.items():
              if not fut.done():
                ...
                if duration > LONG_RECORD*2:
                  ...
                  print(f'Long record ({duration/60:.1f} min): {record["DATA_SOURCE"]} : {record["RECORD_ID"]}')
        ...
        while len(futures) < executor._max_workers:
          try:
            ...
            #print(response)
            if not response:
              print(f'No redo records available. Pausing for {EMPTY_PAUSE_TIME} seconds.')
              empty_pause = time.time() + EMPTY_PAUSE_TIME
              break
            msg = response.decode()
            futures[executor.submit(process_msg, g2, msg, args.info)] = (msg,time.time())
          except Exception as err:
